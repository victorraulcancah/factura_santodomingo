...
WHERE pv.id_venta = '{$row['id_venta']}' ";
    ...
    public function buscarDocInfo()
    {
        ...
        if (strlen($_POST['doc']) == 8) {
            ...
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, 'https://dniruc.apisperu.com/api/v1/dni/' . $_POST['doc'] . '?token=' . $token);
            ...
            if (isset($apiData['nombres'])) {
                $nombreCompleto = trim($apiData['nombres'] . ' ' . $apiData['apellidoPaterno'] . ' ' . $apiData['apellidoMaterno']);
                $respuesta = [
                    "res" => true,
                    "data" => [
                        "nombre" => $nombreCompleto,
                        "nombres" => $apiData['nombres'],
                        "apellido_paterno" => $apiData['apellidoPaterno'],
                        "apellido_materno" => $apiData['apellidoMaterno'],
                        "razon_social" => '',
                        "direccion" => ''
                    ]
                ];
                ...
                $respuesta = [
                    "res" => false,
                    "data" => ["nombre" => '', "razon_social" => '', "direccion" => '']
                ];
            }
